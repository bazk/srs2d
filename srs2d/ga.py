    ...
    def execute(self, run):
        ...
        while (generation < NUM_GENERATIONS):
            ...
            for ind in range(len(self.population)):
                params = self.population[ind].decode()
                self.simulator.set_ann_parameters(ind, params)
            ...
            for d in D:
                for i in range(3):
                    ...
                    for ind in range(len(self.population)):
                        self.population[ind].fitness += fit[ind]
            ...
            while len(new_population) < POPULATION_SIZE:
                if random.random() < CROSSOVER_RATE:
                    ind1 = self.population.select()
                    ind2 = self.population.select()
